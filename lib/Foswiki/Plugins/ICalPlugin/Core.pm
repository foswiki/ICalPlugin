...
sub FORMATICAL {
    ...
    foreach my $entry ( @{ $cal->entries() } ) {
        ...
        my $recurrence =
          ( $entry->property("rrule") || $entry->property("rdate") );
        if ($recurrence) {
            my @recurrence =
              unfoldRecurrence( $props, $params, $queryStart, $queryEnd );
...
#writeDebug("### generated ".scalar(@recurrence)." events from recurrence ".$recurrence->[0]->value);
    ...
    unless ( $theSort eq 'off' ) {
        @events =
          sort { ( $a->{$sortCrit} || 0 ) <=> ( $b->{$sortCrit} || 0 ) }
          @events;
    }
...
#   }
        ...
        next
          if ( defined $event->{_startepoch}
            && $event->{_startepoch} > $queryEndEpoch )
          || ( defined $event->{_endepoch}
            && $event->{_endepoch} < $queryStartEpoch );
        ...
        $line =~
s/\$(?:location|dtstart|start|dtend|until|end|last-modified|modified|description|attendee|organizer|rrule|rdate|summary)//g;
        $line =~
s/\$[mec]?(seco?n?d?s?|minu?t?e?s?|hour?s?|day|w(eek|day)|dow|mo(?:nt?h?)?|ye(?:ar)?)(\(\)|(?=\W|$))//g;
...
sub formatEvent {
    ...
    $format = '   * $day $mon $year - $location - $summary'
      unless defined $format;

    if (   defined $params->{rangeformat}
        && defined $event->{end}
        && $event->{start} != $event->{end} )
    {
        $format = $params->{rangeformat};
        writeDebug(
"found a range: start=$event->{start} - end=$event->{end}: $event->{summary}"
        );
    }
...
sub parseTime {
    ...
    unless ( defined $result ) {
        Foswiki::Func::writeWarning(
            "ICalPlugin::Core - cant parse time '$time'");
        $result = 0;
    }
...
sub formatTime {
    ...
    $value =~
s/\$${prefix}week/Foswiki::Time::_weekNumber($wday, $yday, $year + 1900)/egi;
...
sub unfoldRecurrence {
    ...
    my $start =
      DateTime::Format::ICal->parse_datetime( $props->{dtstart}[0]->value );
...
sub generateEvent {
    ...
    foreach my $key ( keys %$props ) {
        ...
        my $val =
          ( defined $override{$key} ) ? $override{$key} : $property->value;
        $val =~ s/^mailto://i;
        ...
        if ( $key eq 'description' ) {
            ...
            $event{start} = $event{$key} =
              DateTime::Format::ICal->parse_datetime($val);
...
sub getQuerySpan {
    ...
    my $dtStart =
      DateTime->from_epoch(
        epoch => ( defined $theStart ) ? parseTime($theStart) : time );
...
sub updateCalendar {
    ...
    my ( $calendarWeb, $calendarTopic, $calendarAttachment ) =
      $this->getCalendarAttachment;
...
"
    ...
    if (
        Foswiki::Func::attachmentExists(
            $calendarWeb, $calendarTopic, $calendarAttachment
        )
      )
    {
        my $data = Foswiki::Func::readAttachment( $calendarWeb, $calendarTopic,
            $calendarAttachment );
        if ($data) {
            $data = Foswiki::Sandbox::untaintUnchecked($data)
              ;    # SMELL: some libs barf on tainted strings

            my $oldCal =
              Foswiki::Plugins::ICalPlugin::DataICal->new( data => $data )
              ;    # SMELL: do we still need this wrapper?

            if ( ref($oldCal) eq 'Class::ReturnValue' ) {
                print STDERR "ERROR:"
                  . $oldCal->error_message
...
sub getCalendarAttachment {
    ...
    unless ( $this->{calendarWeb} ) {
        my $calendar = Foswiki::Func::getPreferencesValue('ICALPLUGIN_CALENDAR')
          || $Foswiki::cfg{ICalPlugin}{DefaultCalendar};
        $calendar = Foswiki::Func::expandCommonVariables($calendar)
          if $calendar =~ /%[A-Z]+{.+?}%/;
       ...
       #throw Error::Simple("ICALPLUGIN_CALENDAR not defined") unless $calendar;
        ...
        if ( $calendar =~
/^($Foswiki::regex{webNameRegex})\.($Foswiki::regex{topicNameRegex})\.(.*?)$/
          )
        {
            $this->{calendarWeb}        = $1;
            ...
            throw Error::Simple(
"invalid format of ICALPLUGIN_CALENDAR: $calendar, must be web.topic.attachment"
            );
        }
    }

    writeDebug(
"using calendar at web=$this->{calendarWeb} topic=$this->{calendarTopic}, attachment=$this->{calendarAttachment}"
    );

    return (
        $this->{calendarWeb},
        $this->{calendarTopic},
        $this->{calendarAttachment}
    );
}
...
# now, we've found a meta data definition that has got a DataForm associated with it
        my ( $formWeb, $formTopic ) =
          Foswiki::Func::normalizeWebTopicName( $web, $formWebTopic );
        next unless Foswiki::Func::topicExists( $formWeb, $formTopic );

        # now check the TopicType for "EventTopic"
        my $form =
          new Foswiki::Form( $Foswiki::Plugins::SESSION, $formWeb, $formTopic );
...
sub createEvent {
    ...
    if ( defined $params{start} && $params{start} ne '' ) {
        $event{dtstart} =
          DateTime->from_epoch( epoch => parseTime( $params{start} ) );

        my $icalStart =
          DateTime::Format::ICal->format_datetime( $event{dtstart} );
        $icalStart =~ s/T.*//;    # strip of time
        $vevent->add_properties(
            'dtstart' => [ $icalStart, { VALUE => 'DATE' } ] );
    }

    if ( $params{end} ) {
        $event{dtend} =
          DateTime->from_epoch( epoch => parseTime( $params{end} ) );
    ...
    # optional unique
    foreach my $prop
      qw(location organizer priority recurrence-id sequence status transp uid url)
    {
        my $val = $params{$prop};
    ...
    if ( defined $params{freq} && $params{freq} ne 'none' ) {
        ...
        $event{byday} = [ split( /\s*,\s*/, $event{byday} ) ]
          if defined $event{byday};
        $event{bymonth} = [ split( /\s*,\s*/, $event{bymonth} ) ]
          if defined $event{bymonth};
        $event{bymonthday} = [ split( /\s*,\s*/, $event{bymonthday} ) ]
          if defined $event{bymonthday};
        $event{byyearday} = [ split( /\s*,\s*/, $event{byyearday} ) ]
          if defined $event{byyearday};
